...
def train(args):
    ...
    ## Prepare data
    train_dataloader, train_dataset = create_dataloader(args)

    ## Load if exist
    initial_global_step = load_if_exist(args, accelerator, len(train_dataset))

    ## Prepare 3rd party utils
    prepare_3rd_party(args, accelerator, len(train_dataset))
    ## -----------------------------------------------------------------------------------------------
    ...
    for batch in train_dataloader:
            ...
            for _ in range(args.num_steps_fake_dmd):
                ...
                with torch.no_grad(), torch.autocast("cuda", dtype=weight_dtype), transformer_teacher.disable_adapter():
                    model_input = fm_solver.flow_matching_sampling(transformer_teacher, noise,
                                                                ...
                                                                cfg_scale=args.cfg_teacher).half()
                ...
                avg_dmd_fake_loss = fake_diffusion_loss(
                    ...
                    optimizer_fake, lr_scheduler_fake, params_to_optimize_fake,
                    weight_dtype, noise_scheduler,
                    accelerator_fake, args,
                )
        ...
        logs = {
            "fake_loss": avg_dmd_fake_loss.detach().item() if args.do_dmd else 0,
            "dmd_loss": avg_dmd_loss.detach().item() if args.do_dmd else 0,
